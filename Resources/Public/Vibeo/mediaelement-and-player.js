...
mejs.HtmlMediaElementShim = {
	...
	create: function(el, o) {
		...
		if (options.hideContextualMenu === 1) {

			htmlMediaElement.addEventListener('contextmenu', function(event) {
...
'class="mejs-shim"></embed>';
        ...
        const shortUrl = 'youtu.be';
				var
          videoId = playback.url.includes(shortUrl) ? playback.url.substr(playback.url.lastIndexOf('/')+1) : playback.url.substr(playback.url.lastIndexOf('=')+1);
...
(function ($) {
	...
	mejs.MediaElementPlayer.prototype = {
		...
		setPlayerSize: function(width,height) {
			...
			// detect 100% mode - use currentStyle for IE since css() doesn't return percentages
      		if (t.height.toString().indexOf('%') > 0 || t.$node.css('max-width') === '100%' || parseInt(t.$node.css('max-width').replace(/px/,''), 10) / t.$node.offsetParent().width() === 1 || (t.$node[0].currentStyle && t.$node[0].currentStyle.maxWidth === '100%')) {
				...
				var
                    ...
                    parentWidth = t.container.parent().closest(':visible').hasClass('js-fold') ? t.options.videoWidth : t.container.parent().closest(':visible').width(),
			...
			if(t.container.width() < 210){
				t.container.addClass('vibeo-player-small')
			}else{
				t.container.removeClass('vibeo-player-small')
			}
		},
...
(function($) {
	...
	$.extend(MediaElementPlayer.prototype, {
		buildplaypause: function(player, controls, layers, media) {
			var
				t = this,
				btnId = 'btnplaypause-' + player.domNode.id;
				play =
				$('<div class="mejs-button mejs-playpause-button mejs-play" >' +
					'<button id="' + btnId + '" class="btnplaypause" type="button"><span class="visuallyhidden">'+t.options.playText+'</span></button>' +
				'</div>')
				.appendTo(controls)
				.click(function(e) {
					e.preventDefault();
			...
			media.addEventListener('play', function() {
				...
				if ($('#' + btnId + ' span').text() != t.options.pauseText){
					$('#' + btnId + ' span').text(t.options.pauseText);
				}
				$('#' + btnId).parent('.mejs-playpause-button').removeClass('mejs-play').addClass('mejs-pause');

				$('.l-region').html(t.options.pauseText);

			}, false);

			media.addEventListener('pause', function() {

				if ($('#' + btnId + ' span').text() != t.options.playText){
					$('#' + btnId + ' span').text(t.options.playText);
				}
				$('#' + btnId).parent('.mejs-playpause-button').removeClass('mejs-pause').addClass('mejs-play');

				if ($('.mejs-playpause-button.mejs-pause').length > 0) {
					$('.l-region').html(t.options.pauseText);
				} else {
					$('.l-region').html(t.options.playText);
				}
			}, false);
	...
	$.extend(MediaElementPlayer.prototype, {
		...
		buildvolume: function(player, controls, layers, media) {
            ...
            if (mode == 'horizontal') {
                ...
                // horizontal version
                var mute = $('<div class="mejs-button mejs-volume-button ' + (media.muted ? 'mejs-mute' : 'mejs-unmute') + '">' +
                                    '<button type="button" class="mejs-volume-mute" aria-controls="' + t.id + '" aria-live="polite"><span class="visuallyhidden">' + t.options.muteTextMute + '</span></button>' +
            ...
            var volumeSlider = t.container.find('.mejs-volume-slider');
            var volumeTotal = t.container.find('.mejs-volume-total');
            var volumeCurrent = t.container.find('.mejs-volume-current');
            var volumeHandle = t.container.find('.mejs-volume-handle');
            var volumePlus = t.container.find('button.mejs-volume-plus');
            var volumePlusText = t.container.find('button.mejs-volume-plus span.visuallyhidden');
            var volumeMinus = t.container.find('button.mejs-volume-minus');
            var volumeMinusText = t.container.find('button.mejs-volume-minus span.visuallyhidden');
            var volumeButton = t.container.find('button.mejs-volume-mute');
            var volumeButtonText = t.container.find('button.mejs-volume-mute span.visuallyhidden');

			var setVolumeLevel = function(volume) {
				...
				if (mode == 'vertical') {
					...
					// handle
					volumeHandle.css('left', Math.round(16 + newLeft - (volumeHandle.width() / 2)));

					// rezize the current part of the volume bar
					volumeCurrent.width( Math.round(newLeft) );
				}

                setVolume(parseFloat(volume).toFixed(1));
            };
            ...
            var setVolume = function (volume) {
                ...
                if (volume > media.volume) {
                    $('div#liveregion').text(t.options.volumeUpText + ' ' + volumeLevel + '%');
                }

                if (volume < media.volume) {
                    $('div#liveregion').text(t.options.volumeDownText + ' ' + volumeLevel + '%');
                }
            ...
            var muteVolume = function () {
                media.setMuted(true);
                volumeSlider.hide();
                volumeButtonText.text(t.options.muteTextMute);
            ...
            var unmuteVolume = function () {
                ...
                volumeSlider.show();
                volumeButtonText.text(t.options.muteTextUnmute);
                mute.removeClass('mejs-mute').addClass('mejs-unmute');
            };

            var initVolume = function () {

                $('.mejs-container').attr('role', 'application');

                setVolumeLevel(player.options.startVolume);
            ...
            volumeButton.on('mouseenter', function(e) {
                if (!media.muted) {
                    volumeSlider.stop().fadeIn(0);
                }
            });

            volumeButton.on('focus', function(e) {
                volumeButton.attr('aria-live', 'polite');
                if (!media.muted) {
                    volumeSlider.show();
                }
            });

            volumeButton.on('mouseleave', function(e) {
                volumeSlider.stop().fadeOut(300);
            });

            volumeButton.on('blur', function(e) {
                volumeButton.attr('aria-live','off');
            });
            ...
            volumeSlider.on('mouseenter', function(e) {
                volumeSlider.stop().fadeIn(0);
            });

            volumeSlider.bind('mousedown', function(e) {

                if (!$(e.target).hasClass('mejs-volume-plus') && !$(e.target).hasClass('mejs-volume-minus')) {
                    setVolumeLevel(computeVolume(e));
            ...
            volumeSlider.on('mouseleave', function(e) {
                volumeSlider.stop().fadeOut(300);
            });

            volumePlus.bind('click', function(e) {
                setVolumeLevel(Math.min(media.volume + 0.1, 1));
            });

            volumeMinus.bind('click', function(e) {
                setVolumeLevel(Math.max(media.volume - 0.1, 0));
            });
			...
			t.container.parent().on('focus','button.guard',function(){
				...
				if (t.media.pluginType === 'native' || (!t.options.usePluginFullScreen && !mejs.MediaFeatures.isFirefox)) {
					...
					fullscreenBtn.click(function() {
						...
						t.container.find('#btnplaypause-' + player.domNode.id).focus();
						if (isFullScreen) {
							player.exitFullScreen();
							// Remove guards
							t.container.parent().find('button.guard').remove();
